...
Since voltatilities are skewed, it is use log-volatilites which are usually normally distributed. However, to control for instances when volatility is zero $sinh_{-1}$ is used instead of taking the log; moreover, $sinh_{-1}(x) = log(2x)$.

\begin{equation} 
  \sigma_{it} = sinh_{-1}(\sqrt{252*\sigma^2_{it}})
  \label{eq:stdev}
\end{equation}
...
Connectedness requires similar calculations, except that the shocks from IRF are no longer assumed orthogonal. To achieve this, the lower triangular matrix $P$ for $\theat^o_{ij}$ in the numerator is replaced by the covariance matrix of the residuals. The generalized FEVD equation is then,
...
```
## + 13/13 vertices, named, from 9d6686a:
